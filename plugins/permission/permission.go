...
// Init initializes the plugin with the Wails application context and registers
...
func (p *PermissionPlugin) Init(app *wails.Application) error {
	...
	app.RegisterNativeMethod("permissions:result", func(args []json.RawMessage) (interface{}, error) {
		...
		// Emit the result as a Wails event to the frontend.
...
func (p *PermissionPlugin) Request(permission string) (string, error) {
	...
	// Use CallNativePlatform to call into the Android Java bridge.
	// This is Go calling Java.
	result := wails.CallNativePlatform("permissions:request", string(payload))
	return result, nil
}
