...
func (p *DaemonPlugin) Init(app *wails.Application) error {
	p.app = app
	return nil
}
...
func (p *DaemonPlugin) Start(opts Options) (string, error) {
	...
	return wails.CallNativePlatform("daemon:start", string(payload)), nil
}

// Stop stops the background/foreground service.
func (p *DaemonPlugin) Stop() (string, error) {
	return wails.CallNativePlatform("daemon:stop", "{}"), nil
}
